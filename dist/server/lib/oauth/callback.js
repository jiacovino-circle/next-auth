...
function _oAuthCallback() {
  _oAuthCallback = _asyncToGenerator(function* (req) {
    ...
    try {
      ...
      var {
        token_secret
      } = yield client.getOAuthRequestToken(provider.params);

      var _tokens = yield client.getOAuthAccessToken(oauth_token, token_secret, oauth_verifier);

      var _profileData = yield client.get(provider.profileUrl, _tokens.oauth_token, _tokens.oauth_token_secret);
